import type { SemesterEntry, TermEntry, Cohort } from './types.ts';
import {
	GRADE_POINTS,
	isOperationalGrade,
...
/**
 * CGPA from semester SGPAs — the credit-weighted mean of the semesters, which
 * is the same number the course-by-course path produces, since every course in
 * a semester is already weighted inside its SGPA.
 * Semesters with no credits (blank rows, a withdrawn term) drop out.
 */
export function cgpaFromSgpa(terms: TermEntry[]): CgpaResult {
	const real = terms.filter((t) => t.credits > 0);
	const credits = real.reduce((sum, t) => sum + t.credits, 0);
	const weighted = real.reduce((sum, t) => sum + t.credits * (t.sgpa || 0), 0);
	const cgpa = credits ? weighted / credits : 0;

	return {
		cgpa: round2(cgpa),
		percentage: round2(cgpa * 10),
		// without the courses we can't tell a pass from a fail, so the only
		// honest credit figure is what was graded
